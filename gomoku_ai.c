...
// 从给定位置沿指定方向扫描，统计连续相同颜色的棋子数
// 返回：连续棋子数
// end_is_open: 输出参数，表示该方向末端是否开放（空位）
int count_direction(Board* board, int row, int col, int dx, int dy, int color, int* end_is_open) {
    int count = 0;
    int i;
    *end_is_open = 0;
...
int check_five(Board* board, int row, int col, int color) {
    ...
    for (d = 0; d < 4; d++) {
        // 正方向扫描
        int right_count = count_direction(board, row, col, dir_x[d], dir_y[d], color, &right_open);

        // 反方向扫描
        int left_count = count_direction(board, row, col, -dir_x[d], -dir_y[d], color, &left_open);
...
int evaluate_point(Board* board, int row, int col, int color) {
    ...
    for (d = 0; d < 4; d++) {
        // 正方向扫描
        right_count = count_direction(board, row, col, dir_x[d], dir_y[d], color, &right_open);

        // 反方向扫描
        left_count = count_direction(board, row, col, -dir_x[d], -dir_y[d], color, &left_open);
